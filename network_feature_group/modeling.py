...
def _segm_resnet(name, backbone_name, num_classes, output_stride, pretrained_backbone):
    ...
    backbone_rgb = resnet.__dict__[backbone_name](
        pretrained=pretrained_backbone,
        replace_stride_with_dilation=replace_stride_with_dilation)

    backbone_depth = resnet.__dict__[backbone_name](
        pretrained=pretrained_backbone,
        replace_stride_with_dilation=replace_stride_with_dilation)
    ...
    if name=='deeplabv3plus':
        ...
        classifier_add = DeepLabHeadV3Plus(inplanes, low_level_planes, num_classes, aspp_dilate)
    elif name=='deeplabv3':
        return_layers = {'layer4': 'out'}
        classifier_add = DeepLabHead(inplanes , num_classes, aspp_dilate)
    backbone_rgb = IntermediateLayerGetter(backbone_rgb, return_layers=return_layers)
    backbone_depth = IntermediateLayerGetter(backbone_depth, return_layers=return_layers)

    model = DeepLabV3(backbone_rgb, backbone_depth, classifier_add)
    return model
