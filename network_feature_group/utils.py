...
class _SimpleSegmentationModel(nn.Module):
    def __init__(self, backbone_rgb, backbone_depth, classifier):
    ...
    def forward(self, rgbs, depths, masks):
        ...
        features_rgb = self.backbone_rgb(rgb_input) if rgb_input.shape[0] != 0 else None
        features_depth = self.backbone_depth(depth_input) if depth_input.shape[0] != 0 else None
        
        x, rgb_pair, depth_pair, sparse_list, sim_list = self.classifier(features_rgb, features_depth, masks)
        # x = self.classifier(features_rgb, features_depth, masks)
        x = F.interpolate(x, size=input_shape, mode='bilinear', align_corners=False)
        # rgb_out = F.interpolate(rgb_out, size=input_shape, mode='bilinear', align_corners=False)
        # depth_out = F.interpolate(depth_out, size=input_shape, mode='bilinear', align_corners=False)
        return x, rgb_pair, depth_pair, sparse_list, sim_list #, rgb_out, depth_out#, align_pair#, decoup_list
        # return x
